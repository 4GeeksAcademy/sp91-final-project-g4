...
def orders():
    ...
    if request.method == 'GET':
        ...
        if user_role == "customer":  # Cliente solo ve sus órdenes
            customer = db.session.execute(
                db.select(Customers).where(Customers.user_id == user_id)
            ).scalar()
            if not customer:
                return jsonify({"message": "Customer not found"}), 404
            query = query.where(Orders.customer_id == customer.id)

        elif user_role == "provider":  # Proveedor solo ve órdenes asignadas
            provider = db.session.execute(
                db.select(Providers).where(Providers.user_id == user_id)
            ).scalar()
            if not provider:
